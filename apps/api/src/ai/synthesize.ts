...
- hiring.skillsInDemand: top 5-8 technical or functional skills you can infer from job listing data in the raw input. Empty array if no job data.
- hiring.hiringThemes: 1-3 strategic hiring themes (e.g. "Cloud-native migration", "AI/ML investment", "Sales expansion"). Infer from job titles and skills patterns. Empty array if insufficient data.
- hiring.targetRoles: structured list of roles they are actively hiring for, extracted from job listing snippets. For each: title, department (Engineering/Sales/HR/Finance/Operations/Other), seniority (entry/mid/senior/lead/executive), skills[], estimatedCount (null if unknown). Max 8 roles. Empty array if no job data.
- competitors.marketRivals: 2-5 main industry peers of the TARGET COMPANY (not the product). Use competitorNames from raw data if available; infer from industry and description otherwise. trafficComparison is relative to the target company (do rivals have higher/similar/lower web traffic — use general knowledge if no data).
- competitors.displacementTargets: tools the PRODUCT being sold can displace at this company. Cross-reference the product's displacedCompetitors list with the company's tech stack (ats, hris, crm, erp). Only populate if there is a genuine match. displacementAngle must be a specific, evidence-based hook. difficulty: low = clear fit, high = strong incumbent.
- webPresence is NOT generated by Claude — it is passed separately from structured data providers.`
...
export async function synthesizeCompanyCard(params: {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      // Build webPresence from raw provider data (not Claude-generated)
      const webPresence = (rawData.estimatedMonthlyVisits != null ||
        rawData.trafficTrend != null ||
        rawData.domainAgeYears != null)
        ? {
            estimatedMonthlyVisits: rawData.estimatedMonthlyVisits ?? null,
            trafficTrend: rawData.trafficTrend ?? 'unknown',
            domainAgeYears: rawData.domainAgeYears ?? null,
            domainRegistrar: rawData.domainRegistrar ?? null,
            topTrafficCountries: rawData.topTrafficCountries ?? [],
          }
        : null
      ...
      const parsed = CompanyCardSchema.safeParse({
        ...
        // webPresence comes from structured providers, not Claude
        webPresence,
        meta: {
...
function buildFallbackCard(
  ...
  return CompanyCardSchema.parse({
    ...
    buyingSignals: [],
    painPoints: [],
    competitors: { marketRivals: [], displacementTargets: [] },
    webPresence: null,
    engagement: null,
    intent: null,
    synthesis: {
...
function getCardStructure() {
  return {
    ...
    hiring: {
      ...
      skillsInDemand: '["skill1", "skill2", ...] or []',
      hiringThemes: '["theme1", "theme2"] or []',
      targetRoles: '[{ title, department, seniority: entry|mid|senior|lead|executive, skills: string[], estimatedCount: number|null }] or []',
    },
    techStack: {
    ...
    competitors: {
      marketRivals: '[{ name, domain: string|null, positioningNote, trafficComparison: higher|similar|lower|unknown }]',
      displacementTargets: '[{ toolName, category, displacementAngle, difficulty: high|medium|low }]',
    },
    synthesis: {
